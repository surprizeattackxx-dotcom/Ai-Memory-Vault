...
- **[ai-memory-vault.md](ai-memory-vault.md):** the build script. Run it inside Claude and it checks whether Obsidian is even installed yet (and installs it for you if not), interviews you, then builds a complete, self-maintaining system: a boot config, a folder structure around your real projects, daily notes that write themselves, a profile that updates as the AI learns about you, and "Jobs" that teach it to do your recurring tasks your way. Your vault becomes the AI's memory, so it lives outside the model with no size ceiling, and the AI holds only what the current task needs while reaching anything else in one step. It also knows the difference between something you told it and something it inferred, catches contradictions instead of quietly overwriting old facts, and ships with a Memory Health Check job you can run anytime. Watch the walkthrough: [https://www.youtube.com/playlist?list=PLN7lTYpeRLOc](https://www.youtube.com/playlist?list=PLN7lTYpeRLOc)
- **[MEMORY_PROTOCOL.md](MEMORY_PROTOCOL.md):** the model-agnostic contract underneath all of it — the memory classes, the metadata, and the eight operations (boot, retrieve, write, resolve a conflict, health-check, and so on) any capable AI agent implements to work this vault, not just Claude Code. A copy ships inside every vault this builds, at `Resources/MEMORY_PROTOCOL.md`, so it travels with your memory. `templates/CLAUDE.md` is Claude Code's implementation of it.
- **[schema/memory-metadata.schema.yaml](schema/memory-metadata.schema.yaml):** the machine-readable mirror of the protocol's metadata semantics — a JSON Schema validating a note's frontmatter (fields, enums, date/wikilink shapes, lifecycle relationships). Deterministic-verification foundation, no AI in the loop; the protocol stays canonical when the two disagree (version-checked). Fixtures: `tests/fixtures/metadata/`.
