...
> Status: **design contract only — nothing here is implemented.** No index, cache, embedding store, or external database ships with this repo as a result of this document, and none should be inferred from it. This is written before any such implementation exists, specifically so the contract can't be quietly reshaped to fit whatever gets built first. No dependency is added by this document. No protocol behavior changes.
...
Not an implementation plan, not a technology recommendation, not authorization to start building. No dependency, package, service, or code exists as a result of this document. If and when a specific accelerator is actually proposed, it gets evaluated against every invariant above *before* a line of code lands — and, per the standing discipline this project already runs on for any change to `MEMORY_PROTOCOL.md` or its embedded copies, that proposal states the exact change and gets explicit confirmation before anything is built, same as every other protocol-adjacent decision in this repository.
