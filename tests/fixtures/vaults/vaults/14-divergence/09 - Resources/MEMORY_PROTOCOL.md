...
**A note on the version number above:** this file's `version:` tracks the *protocol document itself* (1.0 shipped with project v3.4; 2.0 shipped with the v3.5 hardening pass; 2.1 shipped with the v3.5.1 correctness audit; 2.2 shipped with v3.6's `incompatible` vault state; 2.3 shipped with the machine-readable metadata schema below; 2.4 shipped with the HEALTH_CHECK hardening — the Inspection Manifest and level-coverage evidence requirements described in `HEALTH_CHECK`, which make a health check's coverage claim mechanically checkable; this is 2.5, shipped with the deterministic Job dependency contract — the full resolution table below, which gives every Required/Preferred/Optional dependency state a defined outcome, including lifecycle states the old policy silently overlooked (`candidate`, `deprecated`, absent `memory_status`) and declared freshness windows; 2.6 shipped with the candidate-promotion determinism pass — the promotion test in Candidate memory below, the fail-closed three-predicate contract that turns "independent enough to promote" from a judgment into something an agent must positively establish, with a deny-by-default outcome; 2.7 shipped with an explicit rule for circular supersession — a case a release audit found the mechanical layer had already invented undocumented behavior for, with nothing in this file backing it; this is 2.8, correcting the Source of truth section's own claim that no acceleration layer ships — a release audit found that statement had gone stale the moment the lexical and semantic accelerators actually shipped, several versions before this fix) — it is a separate counter from the project's own release version in `CHANGELOG.md`, not a typo or a drift between the two. When this file's content changes, its version bumps; the project version and this file's version won't always move together.
...
The vault is authoritative. An acceleration layer may speed up retrieval, but if it disappeared entirely, the vault alone must still contain everything needed to reconstruct the system's behavior. The reverse arrangement — an acceleration layer treated as the source of truth, with Markdown as a lossy export of it — is never correct here. As of this version, AI Memory Vault ships two optional acceleration layers — a lexical index and a semantic (embedding-based) index, see `ACCELERATION_LAYER.md` — alongside the unaccelerated baseline (folder indexes, wikilinks, filesystem/full-text search) that remains fully functional with neither installed. Either layer disappearing entirely, or never having been installed, must leave the vault's behavior unchanged apart from search speed — that guarantee, not the presence or absence of an accelerator, is what this section actually requires.
...
**Circular supersession is malformed, not a resolvable state.** Supersession is one-directional by definition — a chain, never a loop. If two notes' `supersedes`/`superseded_by` links form a cycle (A claims to supersede B, B claims to supersede A, directly or through a longer chain), or short of a formal cycle both notes carry `supersedes` pointing at each other while neither is actually marked `memory_status: superseded`, the pairing itself is invalid — it is not evidence that either side is current. Treat the two notes as **genuinely incompatible** claims with no valid supersession between them (see Contradiction classification): do not guess, do not prefer whichever looks more recent or better-sourced, leave both intact and clearly labeled, and ask. Under Job dependency policy this resolves exactly like "two current notes contradict with no supersession between them" — **BLOCK — ambiguous** for a Required dependency.
...
| `supersedes`/`superseded_by` forms a cycle with another note (see Metadata) | BLOCK — ambiguous | BLOCK — ambiguous | BLOCK — ambiguous |
