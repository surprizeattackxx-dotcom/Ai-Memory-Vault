...
**Upgrade path for an older vault (no rebuild required).** A vault is always in exactly one state relative to the current protocol — `legacy` (no v3.5 metadata/rules detected at all), `partial` (some pieces are current, others aren't, but nothing actively contradicts — e.g. `Resources/MEMORY_PROTOCOL.md` is current but `VAULT-INDEX.md`'s operational summary is still an older version), `current` (everything's synchronized), or `incompatible` (two or more present surfaces flatly disagree on what the same term or field default means — not just staleness; see `MEMORY_PROTOCOL.md`'s "Detecting `incompatible`" for the exact test). Determine which by comparing `Resources/MEMORY_PROTOCOL.md`'s `version:` frontmatter against `templates/`'s in this repo, and checking whether `VAULT-INDEX.md`/`CLAUDE.md` carry the matching sections *and* agree with the protocol file on what they say, not just that they say something. **Never assume or claim a vault is fully current without checking — a `partial` vault reported as `current` is worse than an honest `legacy` one, and an `incompatible` vault reported as either is worse still.**
...
- **`incompatible`:** tell the person plainly — "Incompatible protocol state detected: [name the conflicting files and the disputed term/field]." Don't guess which surface is right and don't silently prefer the newer- or older-looking one. Any note whose lifecycle metadata falls under the dispute stays uninterpreted until they say how to reconcile it; work that doesn't touch that metadata proceeds normally. See `MEMORY_PROTOCOL.md`'s "Required behavior when `incompatible`" for the full rule.
...
**Required means required.** A Job's dependencies are declared in its Context section using the deterministic syntax — `[[Note]]` (operational), `[[Note]] (claim)` (currentness required), `[[Note]] (claim, explicitly-confirmed: <N> days)` (currentness plus a declared recency window). Resolution is table-driven, never improvised: a Required dependency that resolves to a block state (missing, malformed, disputed, ambiguous, superseded, or — under a `(claim)` declaration — not clear `current`) stops that Job with the reason named. The block is scoped to the one Job, not the whole session: an unrelated Job (or unrelated work generally) is unaffected. Preferred and Optional degrade gracefully instead — see `MEMORY_PROTOCOL.md`'s Job dependency policy for the exact resolution table.
