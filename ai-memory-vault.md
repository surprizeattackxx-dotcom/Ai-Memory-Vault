...
6. **The cloud-sync check. Run the check every time; never assume, never silently skip it.** Whether their Documents folder is cloud-synced decides what you tell them next, and guessing wrong either alarms them for nothing or leaves the only copy of their agent's memory unprotected.
   - **Windows:** run `powershell -c "[Environment]::GetFolderPath('MyDocuments')"`. A result containing `OneDrive` means Documents is synced; OneDrive redirects the real folder, so this answer is the truth even when a bare `C:\Users\<name>\Documents` also seems to exist.
   - **macOS:** determine whether iCloud's "Desktop & Documents Folders" syncing is on. If the shell evidence is ambiguous, ask the person the question anyone can answer by looking: "in Finder's sidebar, do Desktop and Documents appear under iCloud?" Unknown resolves by asking, never by skipping.
   - **Linux:** almost never synced; skip unless they mention a sync client.

   Then say the sentence that matches the verdict:
   - **Synced is good news, not a warning:** that sync is a free automatic backup of their agent's whole memory. One instruction comes with it: skip Obsidian's paid Sync add-on in that case, one sync service per vault. And one optional click for belt and braces: right-click the vault folder and choose "Keep Downloaded" (Mac) or "Always keep on this device" (Windows) so the cloud never swaps their notes for internet-only placeholders. The deeper edges live in this repo's `TROUBLESHOOTING.md`.
   - **Not synced gets one honest sentence:** the vault lives only on this computer, so a lost or dead machine takes the memory with it; whenever they want a free backup, `TROUBLESHOOTING.md` lists the options (cloud sync, Time Machine or File History, a private GitHub repo).
